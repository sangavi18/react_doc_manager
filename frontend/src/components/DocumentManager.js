...
const DocumentManager = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (editMode) {
      axios.put(`http://localhost:5000/api/documents/${currentDocumentId}`, formData)
        .then(response => {
          setDocuments(documents.map(doc => doc.id === currentDocumentId ? response.data : doc));
          resetForm();
        })
        .catch(error => {
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="document-form">
        ...
        <div className="button-group">
          <button type="submit" className="submit-button">{editMode ? 'Update' : 'Upload'}</button>
          {editMode && <button type="button" className="cancel-button" onClick={resetForm}>Cancel</button>}
        </div>
      </form>
      <br/><br/>
    </div>
        ...
        {documents.map(document => (
          <div key={document.id} className="card">
            ...
            <div className="bottom-section">
              <span className="title">{document.title}</span><br/>
              <a className="do" href={`http://localhost:5000/${document.filePath}`} download>{document.fileName}</a>
              <div className="row">
                <div className="item">
                  <span className="big-text"><a href={`http://localhost:5000/uploads/${document.filename}`} target="_blank" rel="noopener noreferrer">View</a></span>
                </div>
                <div className="item">
                  <span className="big-text" onClick={() => handleEdit(document)}>Edit</span>
                </div>
                <div className="item">
                  <span className="big-text" onClick={() => handleDelete(document.id)}>Delete</span>
                </div>
              </div>
            </div>
          </div>
